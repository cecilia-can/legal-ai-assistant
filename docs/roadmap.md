...
## Phase 2：法律知识库与检索（RAG）

**目标**：构建一个可追溯、可评估的法律知识库，并让系统具备基于来源的法律知识检索与回答能力。

**总体时间**：约 20-25 天

**前置条件**：Phase 1 完成（含 Change 1.9 鉴权与数据隔离——文档、切片和向量数据须从建表起明确 `userId`、`scope` 或 `knowledgeBaseId`；需要先决定哪些资料属于系统公共知识库、哪些资料属于用户私有知识库；Change 1.9.1 OAuth 可选）
...
### Change 2.1：知识库数据模型与向量基础设施

**时间估算**：3 天

**目标**：
在现有 PostgreSQL 上启用 pgvector，建立知识库、文档、切片和向量的基础数据模型。本 Change 只完成基础设施和数据结构，不急于批量生成向量。

**工作内容**：
1. 配置 PostgreSQL pgvector 扩展
2. 扩展 Prisma Schema，建立 `KnowledgeBase`、`Document`、`DocumentChunk` 和 `Embedding` 模型
3. 建立 `Document → DocumentChunk → Embedding` 关系
4. 为文档和切片预留分类、来源、版本、页码、法条编号、解析状态、哈希值和权限范围等字段
5. 记录 Embedding 模型、向量维度和版本，支持后续重新嵌入
6. 创建必要的索引并完成基础存储、查询和迁移测试

**依赖关系**：Phase 1 完成

---

### Change 2.2：多格式文档导入与规范化

**时间估算**：4 天

**目标**：
实现法律文档的导入、解析和统一规范化，为后续结构切片提供稳定的中间表示。

**工作内容**：
1. 建立原始文件清单、文件哈希、重复检测和版本识别流程
2. 实现文档导入 API 或导入任务，原始文件只读保留，不覆盖原件
3. 支持 PDF、DOCX、DOC、HTML 和 TXT 文档解析
4. 为旧版 DOC 提供转换为中间格式的降级路径
5. 检测扫描版 PDF，并为需要 OCR 的文件记录待处理状态
6. 清理 HTML 脚本、导航和样式，保留标题、正文、表格和来源链接
7. 统一输出带标题、章节、条文、表格和页码信息的规范化文档结构
8. 记录解析成功、解析失败、OCR 待处理和需要人工检查等状态

**依赖关系**：Change 2.1

---

### Change 2.3：法律结构切片与质量校验

**时间估算**：4 天

**目标**：
按照法律法规、司法解释、案例和合同模板的结构进行切片，并确保每个切片可追溯到原始文档位置。

**工作内容**：
1. 为法律法规和司法解释实现章、节、条、款、项识别
2. 为案例识别事实、争议焦点、法院观点和判决结果等结构
3. 为合同模板识别合同条款、条款标题和条款编号
4. 实现结构优先、长度兜底的切片策略，避免条款和表格被无意义截断
5. 为每个切片保存文档 ID、章节、条款编号、页码和原文位置
6. 实现切片预览和质量检查，识别乱码、重复页眉页脚、空内容和异常截断
7. 保存切片版本和切片策略版本，支持后续重新切片

**依赖关系**：Change 2.2

---

### Change 2.4：Embedding、向量检索与 RAG

**时间估算**：4 天

**目标**：
对经过质量校验的文档切片生成向量，实现相似度检索、来源追踪和 RAG 回答。

**工作内容**：
1. 选择并配置 Embedding 模型
2. 实现批量向量化、失败重试和任务状态记录
3. 将切片向量写入 pgvector 并创建向量索引
4. 实现相似度检索、分类过滤和知识库范围过滤
5. 实现 RAG Prompt 构建，将文档标题、页码和法条编号传递给模型
6. 将检索结果集成到聊天流程
7. 要求回答尽可能提供对应来源，并处理无相关资料时的拒答或不确定回答
...
3. 实现文档详情、规范化内容和切片预览
4. 实现分类、来源、状态和版本过滤
5. 支持删除、重新解析、重新切片和重新嵌入
6. 展示解析失败、OCR 待处理和索引失败原因

**依赖关系**：Change 2.2、Change 2.3；需要检索调试能力时依赖 Change 2.4
...
**目标**：
建立法律问题测试集，评估并优化检索质量、来源准确性和性能。

**工作内容**：
1. 建立覆盖法律法规、案例和合同模板的检索测试集
2. 评估 Top-K 召回、来源准确率、法条命中率和回答可追溯性
3. 实现混合检索（向量检索 + 关键词检索）
4. 优化切片长度、重叠范围、过滤条件和召回数量
5. 实现重排序（Reranking）并处理重复切片
6. 添加检索调试界面、耗时统计和失败案例记录
...
| Phase 2 | 法律知识库与检索（RAG） | 20-25 天 | 35-44 天 |
| Phase 3 | 合同审查 | 12-15 天 | 47-59 天 |
| Phase 4 | 案件分析 Agent | 18-22 天 | 65-81 天 |
| Phase 5 | 案件工作空间 | 20-25 天 | 85-106 天 |

**总计**：约 85-106 个工作日（约 4-5 个月）
