...
### Change 1.10：消息流虚拟滚动与 Agent UI 预留

**时间估算**：3 天

**目标**：
为消息列表建立支持动态高度、流式输出和历史分页的虚拟滚动能力，并为后续 Agent 的工具调用、执行过程和长结果展示预留 UI 基础。

**工作内容**：
1. 为 `MessageList` 接入虚拟列表，只对可视区域附近的消息进行渲染
2. 支持 Markdown、代码高亮等内容导致的动态消息高度测量与更新
3. 兼容流式输出过程中的消息高度变化，以及发送消息后的自动滚动到底部
4. 加载更早历史消息时保持当前可视位置，避免列表跳动
5. 为工具调用、工具结果和执行状态预留可折叠的消息项展示能力
6. 对超长日志、JSON 和代码结果采用折叠或懒加载，避免一次性渲染全部内容
7. 补充长消息、流式更新、历史加载、滚动定位和移动端场景测试

**范围边界**：
- 本 Change 优先改造消息流，左侧 `ConversationList` 暂不接入虚拟滚动
- 不在本 Change 实现 Agent 执行引擎或具体工具，仅预留消息类型和展示接口

**交付物**：
- 支持动态高度的虚拟化 `MessageList`
- 消息流滚动行为与长内容展示规范
- 流式更新、历史分页和虚拟滚动测试
- Agent 工具调用/执行状态消息项的 UI 扩展点

**依赖关系**：
- Change 1.5（消息持久化与历史分页）
- Change 1.6（Markdown 渲染与代码高亮）
- Change 1.8（自动滚动与整体交互优化）
